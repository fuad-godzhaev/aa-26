...
namespace AiToolsSuite
{
	[Tool]
	public partial class GenerateDialog : Godot.Button
	{
		
		string apiUrl = "http://127.0.0.1:7851/api/";

		private TextEdit _textEdit;
		private OptionButton _voice;
		private OptionButton _lang;

		public List<string> voiceList = new List<string>();

		public override async void _EnterTree()
		{
			Pressed += GenerateClicked;
			_textEdit = GetNode<TextEdit>("../DialogToGenerate");
			_voice = GetNode<OptionButton>("../VoiceContainer/voice");
			_lang = GetNode<OptionButton>("../LangContainer/lang");

			voiceList = await GetVoicesList();
			
			// Add voices to the OptionButton
			_voice.Clear();
			foreach (var voice in voiceList)
				_voice.AddItem(voice);
		...
		{
			GD.Print("Generate dial");
			string voice = _voice.GetItemText(_voice.Selected);
			string lang = _lang.GetItemText(_lang.Selected); // _lang.Text;
			string text = _textEdit.Text;

			GenerateDialogue(text, voice, lang);
		}

		public async Task<List<string>> GetVoicesList()
		{
				...
				using (var reader = new StreamReader(await responseContent.ReadAsStreamAsync()))
				{
					...
					foreach (var voice in N["voices"])
						voiceList.Add(voice.ToString().Replace("[", "").Replace("]", "").Replace('"', ',').Replace(",", "").Replace(".wav", "").Replace(" ", ""));

					return voiceList;
				}
		}

		public async void GenerateDialogue(string line, string voice, string lang)
		{
			var client = new System.Net.Http.HttpClient();

			var requestContent = new FormUrlEncodedContent(new[] {
				new KeyValuePair<string, string>("text_input", line),
				new KeyValuePair<string, string>("text_filtering", "standard"),
				new KeyValuePair<string, string>("character_voice_gen", voice + ".wav"),
				new KeyValuePair<string, string>("narrator_enabled", "true"),
				new KeyValuePair<string, string>("narrator_voice_gen", "male_03.wav"),
				new KeyValuePair<string, string>("text_not_inside", "character"),
				new KeyValuePair<string, string>("language", lang),
				new KeyValuePair<string, string>("output_file_name", "output"),
				new KeyValuePair<string, string>("output_file_timestamp", "true"),
				new KeyValuePair<string, string>("autoplay", "true"),
				new KeyValuePair<string, string>("autoplay_volume", "0.8")
			});
			...
			using (var reader = new StreamReader(await responseContent.ReadAsStreamAsync()))
			{
				string jsonResult = await reader.ReadToEndAsync();
				var N = JSON.Parse(jsonResult);
				string wavPath = N["output_file_path"];
				GD.Print(wavPath);
				...
				string voiceDir = Path.Combine(voicesDir, voice);
				if (!Directory.Exists(voiceDir))
					Directory.CreateDirectory(voiceDir);

				string langDir = Path.Combine(voiceDir, lang);
				...
				if (File.Exists(wavPath))
				{
					...
					string destinationPath = Path.Combine(langDir, fileName);

					File.Move(wavPath, destinationPath);
					GD.Print("File moved to: " + destinationPath);
					...
					if (Engine.IsEditorHint())
					{
						...
						// Add a small delay to ensure the file is indexed
						await Task.Delay(1000);

						// Select the file in the FileSystem dock
						var projectPath = ProjectSettings.GlobalizePath(destinationPath);
						editorInterface.SelectFile(projectPath);
						
						// Optionnellement, ouvrir l'inspecteur
						editorInterface.EditResource(GD.Load<AudioStream>(projectPath));
					}
				}
				else
					GD.Print("Error: Generated audio file not found at " + wavPath);
			}
		}
	}
}
